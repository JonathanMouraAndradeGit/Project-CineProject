...
@Component({
  selector: 'app-form-cad1',
  standalone: false,
  templateUrl: './form-cad1.component.html',
  styleUrl: './form-cad1.component.css'
})
export class FormCad1Component {
  fg: FormGroup
  qtdC = 0
  qtdTot = 0
  ...
  constructor(private fb: FormBuilder, private elementRef: ElementRef, private rend: Renderer2) {
    this.fg = fb.group({
      ...
      gfield2: fb.group({
        nickName: [null, [Validators.nullValidator, this.checkField1()]],
  ...
  checkFieldDate(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (value) {
        if (value < 18) {
          return { msgE: "idade deve ser superior a 18" }
        }
      } else {
        return { msgE: "nao pode ser nulo" }
      }
